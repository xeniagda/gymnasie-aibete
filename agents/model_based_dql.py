...
class RLModel(kr.models.Model):
    def __init__(self):
        ...
        self.layer1 = kr.layers.Dense(64,
                                      bias_initializer=None,
                                      activation=kr.activations.elu)
        self.layer2 = kr.layers.Dense(len(ACTIONS) + AGENT_INPUT_SIZE,
                                      bias_initializer=None,
...
class ModelBasedDeepQlearner:
    ...
    def train_on_random_minibatch(self):
        ...
        return loss_q.numpy(), loss_lvl.numpy()

    def train_on_batch(self, agent_input_before, action, agent_input_after,
                       reward):
        q_after = self.model(agent_input_after)[:,:len(ACTIONS)]
        ...
        with tf.GradientTape() as tape:
            pred = self.model(agent_input_before)
            pred_q_for_all_actions = pred[:,:len(ACTIONS)]
            pred_level = pred[:,len(ACTIONS):]
            ...
            loss_q = self.loss_measure(wanted_q, pred_q_for_action)
            loss_lvl = self.loss_measure(agent_input_after, pred_level)

            gradients = tape.gradient(loss_q + loss_lvl, tvars)
        self.opt.apply_gradients(zip(gradients, tvars))

        self.latestLoss = loss_q + loss_lvl
        return (loss_q, loss_lvl)
